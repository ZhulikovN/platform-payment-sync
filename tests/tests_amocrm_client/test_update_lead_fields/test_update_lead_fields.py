...
async def test_update_all_lead_fields() -> None:
    ...
    try:
        ...
        for field in current_fields:
            ...
            elif settings.AMO_LEAD_FIELD_TOTAL_PAID and field_id == settings.AMO_LEAD_FIELD_TOTAL_PAID:
                current_total_paid = int(values[0]["value"]) if values else 0
                logger.info(f"  {field_name}: {current_total_paid} руб")
            elif len(values) > 1:
        ...
        if settings.AMO_LEAD_FIELD_TOTAL_PAID:
            logger.info(f"Текущий общий оплаченный итог: {current_total_paid} руб")
...
Источник: integration_test"""
        ...
        for field in updated_fields:
            ...
            if field_id == settings.AMO_LEAD_FIELD_PURCHASE_COUNT:
                ...
                final_total_paid = int(values[0]["value"]) if values else 0
                logger.info(f"  {field_name}: {final_total_paid} руб (было {current_total_paid})")
                assert final_total_paid == current_total_paid + 5000, "Общий итог должен увеличиться на 5000"
